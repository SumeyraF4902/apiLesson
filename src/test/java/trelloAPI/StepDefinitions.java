...
public class StepDefinitions {
   ...
   public static Response response;
    ...
    public void kullanıcı_board_create_edebilmek_icin_ilgili_url_e_POST_methodu_ve_ilgili_endpointler_ile_request_atar(String idPath, String id, String boardsPath, String boards) {
        ...
        String idPathFormated = String.format("{%s}",idPath); //{%s} bu ne ise yarar idPath {} icine almaya yarar
        System.out.println("idPathFormated: " + idPathFormated); //{idPath}

        String boardsPathFormated = String.format("{%s}",boardsPath);// //{%s} bu ne ise yarar boardsPath {} icine almaya yarar

        response=given().
                spec(specification).
                contentType(ContentType.JSON).
                when().
                post("/" + idPathFormated +"/" +boardsPathFormated + "/");     // post("/{idPath}/{boardsPath}")  post body genellikle request body  ister ama arada istemediği örneklerde  vardır
    ...
    @Then("Kullanıcı listin basarı ile create edildigini verify eder")
    public void kullanıcı_listin_basarı_ile_create_edildigini_verify_eder() {
        response.
                then().
                assertThat()
                .statusCode(200);
